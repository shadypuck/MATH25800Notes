...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item Thus, $k$ is the minimum degree that the minimal polynomial can have.
            \item To recap, at this point, we have proven that $g\in\ker(\rho)$ and no element of $\ker(\rho)$ besides 0 has lesser degree.
            \item We now prove that $\ker(\rho)=(g)$. Let $f\in\ker(\rho)$. Then $f=qg+r$ by the Euclidean algorithm for monic polynomials, where $\deg(r)<k$. Additionally,
            \begin{equation*}
                0 = f(T)v
                = q(T)g(T)v+r(T)v
                = q(T)\cdot 0+r(T)v
        ...
        \end{itemize}
        \item Let $h\in\Zg$ be arbitrary. Then
        \begin{align*}
            0 &= T^h(0)\\
            &= T^hg(T)v\\
            &= T^h(T^k-(a_{k-1}T^{k-1}+\cdots+a_1T+a_0))v\\
            &= (T^{h+k}-(a_{k-1}T^{h+k-1}+\cdots+a_1T^{h+1}+a_0T^h))\\
            &= (T^k-(a_{k-1}T^{k-1}+\cdots+a_1T+a_0))T^hv\\
            &= g(T)T^hv
        \end{align*}
        as well.
        \begin{itemize}
            \item We can also rationalize $g(T)T^h=T^hg(T)$ by pulling everything back to the domain of polynomials and noting that polynomial multiplication is commutative.
        \end{itemize}
        \item It follows that $g(T)w=0$ for all $w\in W$.
    ...
    \end{itemize}
    \item We now summarize many of the above results (and a further one to come) in the following lemma.
    \begin{itemize}
