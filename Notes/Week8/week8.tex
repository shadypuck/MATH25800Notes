...
\begin{itemize}
    ...
    \item Recall:
    \begin{itemize}
        \item We have shown that $\bigwedge(M)$, where $M$ is a free $R$-module with $S$ as basis, equals $F_R(S)/I$.
    ...
    \begin{itemize}
        ...
        \end{figure}
        \item Then $\bigwedge(Re_1\oplus\cdots\oplus Re_n)$ is a free $R$-module with the $e_I$ as basis.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Let $\bigwedge(N)=A$ be an $R$-algebra.
    ...
    \end{itemize}
    \item We have that $\bigwedge(M)=\bigoplus_{k\geq 0}\bigwedge^k(M)$.
    \begin{itemize}
        \item Thus, $\rank(M)=n$ where $\bigwedge^k(M)=0$ for all $k>n$.
    \end{itemize}
    \item Corollary of the universal property: Given $L:M\to N$ an $R$-module homomorphism, then there is a unique $R$-algebra homomorphism called $\bigwedge(L)$ such that the following diagram commutes.
    \emph{picture}
    \item What does $\bigwedge^k(M)$ mean?
    \begin{itemize}
        \item It is an $R$-module spanned by $e_I$, where the cardinality of $I$ is $k$. This is a bad definition, though.
        \item Better: $R$-submodule of $\bigwedge(R)$ generated by $v_1,\dots,v_k$ where all $v_i\in M$.
    \end{itemize}
    \item Let $\bigwedge(M)=\bigoplus_{k\geq 0}\bigwedge^k(M)$. Given $L:M\to N$, we get $R$-module homomorphisms called $\bigwedge^kL:\bigwedge^kM\to\bigwedge^kN$ and $\bigwedge(L)(\alpha_0+\alpha_1+\cdots)=\alpha_0+L(\alpha_1)+\bigwedge^2L(\alpha_2)+\cdots$.
    \item Now assume that $\rank(M)=\rank(N)=n$ and $L:M\to N$ is an $R$-module homomorphism. Then $\bigwedge^n(L):\bigwedge^n(M)\to\bigwedge^n(N)$. This is also called the determinant of $L$.
    \item Now assume that $M=N$. We have a map from a space to itself?? Suppose $\omega$ is a basis element of $\bigwedge^nM$. Then $\bigwedge^n(L)\omega=\alpha\omega$. There exists such an $\alpha\in R$.
    \begin{itemize}
        \item Then $\bigwedge^n(L)(a\omega)=a\alpha\omega=\alpha(a\omega)$ for all $a\in R$ and $\bigwedge^n(L)\eta=\alpha\eta$ for all $\eta\in\bigwedge^n(M)$.
        \item Let $\det(L)\in R$. Then $\bigwedge^n(L)\eta=(\det(L))\eta$ for all $\eta\in\bigwedge^n(M)$.
    \end{itemize}
    \item On the formula for the inverse in terms of adjoint matrices.
    \begin{itemize}
        \item We have $\bigwedge^{n-1}(L):\bigwedge^{n-1}(M)\to\bigwedge^{n-1}(M)$.
    \end{itemize}
